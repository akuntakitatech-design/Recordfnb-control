...
type Page = 'dashboard' | 'organization' | 'items' | 'partners' | 'purchase' | 'cash-in' | 'cash-out' | 'item-usage' | 'finance' | 'coa-standard' | 'transactions' | 'control' | 'access' | 'settings';
...
const pageTitles: Record<Page, string> = {
  ...
  purchase: 'Invoice Pembelian', 'cash-in': 'Kas & Bank Masuk', 'cash-out': 'Kas & Bank Keluar', 'item-usage': 'Pemakaian Barang', finance: 'Finance & Accounting Master',
...
function Login({ onLogin }: { onLogin: () => void }) {
  ...
  return <main className="login-page">
    ...
    <section className="login-card-wrap"><form className="login-card" onSubmit={submit}>
      ...
      <button className="primary-button" disabled={loading}>{loading ? 'Memeriksa...' : 'Masuk'}</button><small>Foundation v0.11 · Development</small>
    </form></section>
  </main>;
}
...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <section className="content-shell">
      <header className="topbar"><div><span className="eyebrow">FOUNDATION v0.11</span><h2>{pageTitles[active]}</h2></div><div className="user-box"><div className="avatar">{session.user.fullName.slice(0,1).toUpperCase()}</div><div><strong>{session.user.fullName}</strong><span>{session.user.email}</span></div></div></header>
...
function Dashboard({ summary, setActive, clientMode }: { summary: Summary | null; setActive: (v: Page) => void; clientMode:boolean }) {
  if (clientMode) return <div className="page-content">
    ...
    <section className="section-card"><div className="section-title"><div><span className="eyebrow">ALUR KERJA CLIENT</span><h3>Portal client dibuat sederhana</h3></div></div><div className="foundation-list">
      <div><b>01</b><span><strong>Finance Control</strong><small>Invoice Pembelian, Kas/Bank Masuk-Keluar dan Pemakaian Barang adalah area kerja harian client. Tidak ada pilihan COA atau jurnal.</small></span></div>
      ...
      <div><b>03</b><span><strong>Pemakaian Barang</strong><small>Qty mengurangi saldo lokasi dan dinilai otomatis menggunakan HPP moving average.</small></span></div>
      <div><b>04</b><span><strong>Kontrol Exception</strong><small>Saldo yang akan minus diberi warning dan hanya bisa di-override oleh Finance Manager atau Akuntakita.</small></span></div>
    </div></section>
  </div>;
  ...
  return <div className="page-content">
    ...
    <section className="section-card"><div className="section-title"><div><span className="eyebrow">FOUNDATION v0.11</span><h3>Struktur kerja sistem</h3></div></div><div className="foundation-list">
      <div><b>01</b><span><strong>Finance Control</strong><small>Invoice pembelian, kas/bank masuk-keluar dan pemakaian barang menjadi area transaksi bisnis.</small></span></div>
      ...
      <div><b>03</b><span><strong>Persediaan</strong><small>Pemakaian memakai HPP moving average, update saldo per lokasi dan jurnal otomatis dari mapping kategori.</small></span></div>
