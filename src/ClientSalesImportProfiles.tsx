...
type Profile={
  id:string;company_id:string;name:string;provider:string|null;status:'ACTIVE'|'INACTIVE';file_mode:'WIDE'|'VERTICAL';header_row:number;
  delimiter:string;date_format:string;number_format:string;header_signature:string[];column_mapping:Record<string,string>;
...
export function ClientSalesImportProfiles(){
  ...
  const [fileMode,setFileMode]=useState<'WIDE'|'VERTICAL'>('WIDE');
  ...
  const headers=useMemo(()=>splitHeader(headerText,delimiter),[headerText,delimiter]);
  const visibleFields=useMemo(()=>fields.filter(x=>fileMode==='WIDE'?!x.verticalOnly:!x.wideOnly),[fileMode]);
  ...
  function edit(p:Profile){
    setProfileId(p.id);setName(p.name);setProvider(p.provider||'');setFileMode(p.file_mode);setHeaderRow(String(p.header_row));setDelimiter(p.delimiter);setDateFormat(p.date_format);setNumberFormat(p.number_format);setMapping(p.column_mapping||{});setStatus(p.status);setHeaderText((p.header_signature||[]).join('\t'));setError('');setMessage('');
  }
  ...
  async function save(){
    ...
    if(!headers.length)return setError('Tempel satu baris header dari file POS agar signature template tersimpan.');
    if(missingRequired.length)return setError(`Mapping wajib belum lengkap: ${missingRequired.map(x=>x.label).join(', ')}.`);
    setSaving(true);
    try{
      const body={companyId,name:name.trim(),provider:provider.trim()||null,status,fileMode,headerRow:Number(headerRow||1),delimiter,dateFormat,numberFormat,headerSignature:headers,columnMapping:mapping,settings:{version:'0.16A'}};
      ...
      setMessage('Template import POS tersimpan. Header yang sama nantinya dapat dikenali otomatis.');
      await loadProfiles();
  ...
  return <div className="page-content sales-import-profile-page">
    <section className="section-card">
      <div className="section-title master-heading"><div><span className="eyebrow">IMPORT PROFILE · POS</span><h3>Template Mapping Format POS</h3><p>Satu format internal, banyak format file. Mapping disimpan per company agar import berikutnya tidak perlu disusun ulang.</p></div><button className="secondary-button compact" onClick={reset}><Plus size={15}/> Template Baru</button></div>
      ...
      <div className="sales-head-grid">
        ...
        <div className="sales-setup-status"><strong>{profiles.filter(x=>x.status==='ACTIVE').length} template aktif</strong><span>Item alias dan payment alias akan dipakai pada tahap normalisasi.</span></div>
      </div>
      <div className="stock-opname-header">
        ...
        <label>Provider<input value={provider} onChange={e=>setProvider(e.target.value)} placeholder="Olsera / ESB / Custom"/></label>
        <label>Pola Pembayaran<select value={fileMode} onChange={e=>setFileMode(e.target.value as 'WIDE'|'VERTICAL')}><option value="WIDE">Melebar · Cash/QRIS/GoFood per kolom</option><option value="VERTICAL">Vertikal · Payment Type + Amount</option></select></label>
