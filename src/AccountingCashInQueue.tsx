...
type PendingCashIn={ id:string;transaction_number:string;transaction_date:string;reference_number:string|null;cash_in_type:string;source_name:string|null;notes:string|null;grand_total:string;location_name?:string;financial_account_name?:string;attachment_count:number;lines:PendingLine[] };
const money=(value:string|number)=>Number(value||0).toLocaleString('id-ID',{ maximumFractionDigits:0 });

export function AccountingCashInQueue({ companyId,onJournalCreated }:{ companyId:string;onJournalCreated:()=>void }) {
  const [pending,setPending]=useState<PendingCashIn[]>([]);
  ...
  async function load() {
    ...
    try {
      const [p,a]=await Promise.all([
        api<PendingCashIn[]>(`/api/accounting-cash-ins/pending?companyId=${encodeURIComponent(companyId)}`),
        api<Account[]>(`/api/master/accounts?companyId=${encodeURIComponent(companyId)}`),
      ]);
      setPending(p);setAccounts(a);
    } catch (err) { setError(err instanceof Error?err.message:'Gagal memuat antrean penerimaan'); }
  }

  useEffect(()=>{ setAssignments({});void load(); },[companyId]);

  async function direct(tx:PendingCashIn) {
    const txAssignments=tx.lines.map(line=>({ lineId:line.id,accountId:assignments[line.id]||'' }));
    if (txAssignments.some(x=>!x.accountId)) return setError('Pilih akun sumber untuk seluruh baris penerimaan sebelum membuat jurnal.');
    setSavingId(tx.id);setError('');
    try {
      await api(`/api/accounting-cash-ins/${tx.id}/direct`,{ method:'POST',body:JSON.stringify({ assignments:txAssignments }) });
  ...
  return <section className="section-card account-direction-box">
    ...
    {pending.length===0&&!error&&<div className="empty-state"><strong>Tidak ada penerimaan yang menunggu arah akun</strong><span>Kas/Bank masuk yang sudah Finance Verified akan muncul di sini.</span></div>}
  </section>;
}
