...
export default function ClientSalesImportPage({ canVerify, canOverride }:{ canVerify:boolean; canOverride:boolean }) {
  ...
  useEffect(()=>{
    ...
    setBatchPage(1); void loadBatches(companyId,1);
  },[companyId,locations]);
  ...
  function applyPaste() { const parsed=parseTable(paste); if(!parsed.length){setError('Data belum terbaca. Pastikan baris pertama adalah header spreadsheet.');return;} setRows(mapItems(parsed)); setEditorPage(1); setSourceType('PASTE'); setSourceName('Paste spreadsheet'); setError(''); }
  async function uploadFile(file:File) { const text=await file.text(); const parsed=parseTable(text); if(!parsed.length){setError('CSV/TXT belum terbaca.');return;} setRows(mapItems(parsed)); setEditorPage(1); setSourceType('CSV'); setSourceName(file.name); setPaste(''); setError(''); }
  function addRow(){ setRows(v=>[...v,blankRow()]); setEditorPage(Math.max(1,Math.ceil((rows.length+1)/EDITOR_PAGE_SIZE))); setSourceType('MANUAL'); setSourceName('Input manual'); }
  ...
  async function saveBatch(){
    ...
    try {
      ...
      setMessage(`Batch ${result.batchNumber} tersimpan. Jalankan Preview sebelum Finance Verified.`); setRows([]); setEditorPage(1); setPaste(''); await loadBatches(companyId,1);
  ...
  return <div className="page-content sales-import-page">
    <section className="section-card">
      <div className="section-title master-heading"><div><span className="eyebrow">FINANCE CONTROL · PENJUALAN</span><h3>Data Penjualan / Import POS</h3><p>Tempel data dari Excel/Google Sheets atau upload CSV/TXT. Tampilan dibatasi 50 baris per halaman agar tetap ringan meski batch besar.</p></div><button className="secondary-button compact" onClick={()=>{void loadContext();void loadBatches(companyId,batchPage);}}><RefreshCw size={15}/> Refresh</button></div>
      ...
      <div className="helper-box">Header yang dikenali: TANGGAL, NO INVOICE, CASHIER, TYPE, KODE BARANG, NAMA BARANG, QTY, HARGA, DISKON, TOTAL, CASH, QRIS, TRANSFER, COMPLIMENT, GO FOOD, GRAB FOOD. Item yang sudah cocok tidak lagi membuat dropdown master berulang.</div>
