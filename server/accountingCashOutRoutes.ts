...
import { directOperationalCashOut } from './cashOutEngine.js';

export const accountingCashOutRouter = Router();
accountingCashOutRouter.use(requireAuth);

function text(value:unknown) { return String(value ?? '').trim(); }

accountingCashOutRouter.get('/pending', async (req,res) => {
  ...
  const result=await query(
    `SELECT t.id,t.transaction_number,t.transaction_date,t.reference_number,t.payee_name,t.notes,t.grand_total::text,
      ...
      WHERE t.company_id=$1 AND t.transaction_type='CASH_OUT' AND t.cash_out_type='OPERATIONAL_EXPENSE'
...
accountingCashOutRouter.post('/:transactionId/direct', async (req,res) => {
  const transactionId=text(req.params.transactionId);
  const tx=await query<{ company_id:string }>(
    `SELECT company_id FROM transaction_headers WHERE id=$1 AND transaction_type='CASH_OUT'`,[transactionId],
  );
  ...
  try {
    const result=await directOperationalCashOut(transactionId,req.sessionUser!.id,assignments);
    res.json({ ok:true,...result });
  } catch (error) {
    console.error('Direct cash out accounts failed:',error);
    res.status(400).json({ error:error instanceof Error ? error.message : 'DIRECT_ACCOUNTS_FAILED' });
  }
});
