...
async function nextProductionNumber(client:any,companyId:string,date:string) {
  ...
  return `PRD-${year}-${String(r.rows[0].last_number).padStart(5,'0')}`;
}

async function toBaseQuantity(client:any,workspaceId:string,itemId:string,unitId:string,baseUnitId:string,quantity:Decimal) {
  if(unitId===baseUnitId) return quantity;
  const r=await client.query<{multiplier:string}>(
  ...
  if(!r.rowCount) throw new Error('UNIT_CONVERSION_REQUIRED');
  return quantity.mul(r.rows[0].multiplier);
}
...
clientBomProductionRouter.get('/bom-context',async(req,res)=>{
  ...
  const [items,units]=await Promise.all([
    ...
    query(`SELECT id,code,name,decimal_precision FROM units WHERE workspace_id=$1 AND status='ACTIVE' ORDER BY code`,[company.rows[0].workspace_id]),
  ]);
...
clientBomProductionRouter.get('/boms',async(req,res)=>{
  const companyId=text(req.query.companyId),bomType=text(req.query.bomType).toUpperCase();
  if(!companyId)return res.status(400).json({error:'COMPANY_REQUIRED'});
  if(!(await canAccessCompany(req.sessionUser!.id,companyId)))return res.status(403).json({error:'FORBIDDEN_COMPANY'});
  ...
  const ids=headers.rows.map((row:any)=>row.id);
  ...
  const byBom=new Map<string,any[]>();
  for(const line of lines.rows){const current=byBom.get(line.bom_id)||[];current.push(line);byBom.set(line.bom_id,current);}
  res.json(headers.rows.map((row:any)=>({...row,lines:byBom.get(row.id)||[]})));
});

clientBomProductionRouter.post('/boms',async(req,res)=>{
  const companyId=text(req.body?.companyId),bomType=text(req.body?.bomType).toUpperCase();
  const outputItemId=text(req.body?.outputItemId),outputUnitId=text(req.body?.outputUnitId),notes=nullable(req.body?.notes);
  ...
  const company=await query<{workspace_id:string}>(`SELECT workspace_id FROM companies WHERE id=$1 AND status='ACTIVE'`,[companyId]);
  if(!company.rowCount)return res.status(404).json({error:'COMPANY_NOT_FOUND'});
  if(!(await canWriteOperationalWorkspaceMaster(req.sessionUser!.id,company.rows[0].workspace_id)))return res.status(403).json({error:'FORBIDDEN'});

  const client=await pool.connect();
  try{
    await client.query('BEGIN');
    const output=await client.query<{base_unit_id:string;can_sell:boolean;can_produce:boolean;track_stock:boolean}>(
      `SELECT base_unit_id,can_sell,can_produce,track_stock FROM items WHERE id=$1 AND workspace_id=$2 AND status='ACTIVE'`,
    ...
    if(bomType==='PRODUCTION'&&(!output.rows[0].can_produce||!output.rows[0].track_stock))throw new Error('PRODUCTION_OUTPUT_MUST_BE_STOCK_PRODUCIBLE');
    ...
    for(let index=0;index<rawLines.length;index+=1){
      const line=rawLines[index];const componentItemId=text(line.componentItemId),unitId=text(line.unitId),quantity=num(line.quantity),wastePercent=num(line.wastePercent||0);
      if(!componentItemId||!unitId||!Number.isFinite(quantity)||quantity<=0||!Number.isFinite(wastePercent)||wastePercent<0||wastePercent>100)throw new Error(`INVALID_BOM_LINE_${index+1}`);
      if(componentItemId===outputItemId)throw new Error(`BOM_OUTPUT_CANNOT_BE_COMPONENT_LINE_${index+1}`);
      if(used.has(componentItemId))throw new Error(`DUPLICATE_BOM_COMPONENT_LINE_${index+1}`);used.add(componentItemId);
      const component=await client.query<{base_unit_id:string;can_use_in_recipe:boolean;track_stock:boolean}>(
        `SELECT base_unit_id,can_use_in_recipe,track_stock FROM items WHERE id=$1 AND workspace_id=$2 AND status='ACTIVE'`,
        [componentItemId,company.rows[0].workspace_id],
      );
      if(!component.rowCount||!component.rows[0].can_use_in_recipe||!component.rows[0].track_stock)throw new Error(`INVALID_BOM_COMPONENT_LINE_${index+1}`);
    ...
    for(let index=0;index<normalized.length;index+=1){const line=normalized[index];await client.query(
    ...
    await client.query('COMMIT');res.status(201).json({id:header.rows[0].id,version:header.rows[0].version});
  }catch(error){await client.query('ROLLBACK');console.error('Save BOM failed:',error);res.status(400).json({error:error instanceof Error?error.message:'SAVE_BOM_FAILED'});}finally{client.release();}
});
...
clientBomProductionRouter.delete('/boms/:bomId',async(req,res)=>{
  ...
  await query(`INSERT INTO audit_logs(workspace_id,user_id,entity_type,entity_id,action,after_data) VALUES($1,$2,'BOM',$3,'DEACTIVATE_BOM',$4::jsonb)`,[bom.rows[0].workspace_id,req.sessionUser!.id,bomId,JSON.stringify({status:'INACTIVE'})]);
  res.json({ok:true});
});
...
clientBomProductionRouter.post('/productions',async(req,res)=>{
  const companyId=text(req.body?.companyId),locationId=text(req.body?.locationId),bomId=text(req.body?.bomId),transactionDate=text(req.body?.transactionDate),notes=nullable(req.body?.notes);
  const batchCount=num(req.body?.batchCount),actualOutput=num(req.body?.actualOutput);
  if(!companyId||!locationId||!bomId||!/^\d{4}-\d{2}-\d{2}$/.test(transactionDate)||!Number.isFinite(batchCount)||batchCount<=0||!Number.isFinite(actualOutput)||actualOutput<=0)return res.status(400).json({error:'PRODUCTION_REQUIRED_FIELDS'});
  ...
  const company=await query<{workspace_id:string}>(`SELECT workspace_id FROM companies WHERE id=$1 AND status='ACTIVE'`,[companyId]);if(!company.rowCount)return res.status(404).json({error:'COMPANY_NOT_FOUND'});
  const location=await query(`SELECT 1 FROM locations WHERE id=$1 AND company_id=$2 AND status='ACTIVE'`,[locationId,companyId]);if(!location.rowCount)return res.status(400).json({error:'LOCATION_OUTSIDE_COMPANY'});
  ...
  try{
    ...
    const bomLines=await client.query<{id:string;component_item_id:string;quantity:string;unit_id:string;waste_percent:string;base_unit_id:string;component_name:string}>(
      `SELECT bl.id,bl.component_item_id,bl.quantity::text,bl.unit_id,bl.waste_percent::text,i.base_unit_id,i.name component_name
        ...
        WHERE bl.bom_id=$1 ORDER BY bl.line_no`,[bomId],
    );
    ...
    for(let index=0;index<bomLines.rows.length;index+=1){const line=bomLines.rows[index];const required=productionRequirement(line.quantity,line.waste_percent,batchCount);const base=await toBaseQuantity(client,company.rows[0].workspace_id,line.component_item_id,line.unit_id,line.base_unit_id,required);await client.query(
      `INSERT INTO transaction_lines(transaction_id,line_no,line_type,item_id,description,quantity,unit_id,unit_price,gross_amount,dpp_amount,tax_amount,line_total,location_id,metadata)
    ...
    await client.query(`INSERT INTO audit_logs(workspace_id,user_id,entity_type,entity_id,action,after_data) VALUES($1,$2,'TRANSACTION',$3,'CLIENT_CREATE_PRODUCTION',$4::jsonb)`,[company.rows[0].workspace_id,req.sessionUser!.id,header.rows[0].id,JSON.stringify({transactionNumber,bomId,batchCount,standardOutput:standardBase.toFixed(6),actualOutput:actualBase.toFixed(6),yieldPercent:yieldPct.toFixed(6),lineCount:bomLines.rows.length+1})]);
    await client.query('COMMIT');res.status(201).json({...header.rows[0],standardOutput:standardBase.toFixed(6),actualOutput:actualBase.toFixed(6),yieldPercent:yieldPct.toFixed(6)});
  }catch(error){await client.query('ROLLBACK');console.error('Create production failed:',error);res.status(400).json({error:error instanceof Error?error.message:'CREATE_PRODUCTION_FAILED'});}finally{client.release();}
});

clientBomProductionRouter.get('/productions/:transactionId/preview',async(req,res)=>{
  const id=text(req.params.transactionId);const tx=await query<{company_id:string;location_id:string|null}>(`SELECT company_id,location_id FROM transaction_headers WHERE id=$1 AND transaction_type='PRODUCTION'`,[id]);if(!tx.rowCount)return res.status(404).json({error:'PRODUCTION_NOT_FOUND'});if(!(await canVerifyTransaction(req.sessionUser!.id,tx.rows[0].company_id)))return res.status(403).json({error:'FINANCE_VERIFY_ROLE_REQUIRED'});if(!tx.rows[0].location_id||!(await canAccessLocation(req.sessionUser!.id,tx.rows[0].location_id)))return res.status(403).json({error:'LOCATION_FORBIDDEN'});try{res.json(await previewProduction(id));}catch(error){res.status(400).json({error:error instanceof Error?error.message:'PREVIEW_PRODUCTION_FAILED'});}
});

clientBomProductionRouter.post('/productions/:transactionId/verify',async(req,res)=>{
  const id=text(req.params.transactionId),allowBelowZero=Boolean(req.body?.allowBelowZero);const tx=await query<{company_id:string;location_id:string|null}>(`SELECT company_id,location_id FROM transaction_headers WHERE id=$1 AND transaction_type='PRODUCTION'`,[id]);if(!tx.rowCount)return res.status(404).json({error:'PRODUCTION_NOT_FOUND'});if(!(await canVerifyTransaction(req.sessionUser!.id,tx.rows[0].company_id)))return res.status(403).json({error:'FINANCE_VERIFY_ROLE_REQUIRED'});if(!tx.rows[0].location_id||!(await canAccessLocation(req.sessionUser!.id,tx.rows[0].location_id)))return res.status(403).json({error:'LOCATION_FORBIDDEN'});if(allowBelowZero&&!(await canOverride(req.sessionUser!.id,tx.rows[0].company_id)))return res.status(403).json({error:'INVENTORY_OVERRIDE_MANAGER_REQUIRED'});try{res.json({ok:true,...await verifyProduction(id,req.sessionUser!.id,allowBelowZero)});}catch(error){console.error('Verify production failed:',error);res.status(400).json({error:error instanceof Error?error.message:'VERIFY_PRODUCTION_FAILED'});}
});
...
clientBomProductionRouter.get('/inventory-control',async(req,res)=>{
  ...
  const r=await query(
    `SELECT i.id item_id,i.code,i.name,c.name category_name,u.code unit_code,
            ...
            lm.movement_type last_movement_type,lm.created_at last_movement_at,
            CASE WHEN COALESCE(ib.quantity_on_hand,0)<0 THEN 'STOK MINUS' WHEN COALESCE(ib.quantity_on_hand,0)=0 THEN 'HABIS' ELSE 'OK' END status
       FROM companies co
       ...
       LEFT JOIN LATERAL (SELECT movement_type,created_at FROM inventory_movements im WHERE im.company_id=co.id AND im.location_id=$2 AND im.item_id=i.id ORDER BY im.created_at DESC,im.id DESC LIMIT 1) lm ON TRUE
...
clientBomProductionRouter.get('/stock-card',async(req,res)=>{
  const companyId=text(req.query.companyId),locationId=text(req.query.locationId),itemId=text(req.query.itemId),from=text(req.query.from),to=text(req.query.to);
  if(!companyId||!locationId||!itemId)return res.status(400).json({error:'COMPANY_LOCATION_ITEM_REQUIRED'});
  if(!(await canAccessCompany(req.sessionUser!.id,companyId))||!(await canAccessLocation(req.sessionUser!.id,locationId)))return res.status(403).json({error:'FORBIDDEN'});
  const opening=from?await query<{quantity_after:string}>(`SELECT quantity_after::text FROM inventory_movements WHERE company_id=$1 AND location_id=$2 AND item_id=$3 AND created_at<$4::date ORDER BY created_at DESC,id DESC LIMIT 1`,[companyId,locationId,itemId,from]):{rows:[]} as any;
  const r=await query(
    `SELECT im.id,im.movement_type,im.quantity::text,im.unit_cost::text,im.movement_value::text,im.quantity_after::text,im.average_cost_after::text,im.created_at,
      ...
      WHERE im.company_id=$1 AND im.location_id=$2 AND im.item_id=$3
        AND ($4='' OR t.transaction_date>=$4::date) AND ($5='' OR t.transaction_date<=$5::date)
  ...
  res.json({openingQuantity:opening.rows[0]?.quantity_after||'0',rows:r.rows.map((row:any)=>({...row,direction:inbound.has(row.movement_type)?'IN':'OUT',signed_quantity:(inbound.has(row.movement_type)?new Decimal(row.quantity):-new Decimal(row.quantity)).toFixed(6)}))});
});
