...
import { clientStockOpnameRouter } from './clientStockOpnameRoutes.js';
import { clientBomProductionRouter } from './clientBomProductionRoutes.js';
...
app.get('/api/health', async (_req, res) => {
  const db = await query<{ now: string }>('SELECT CAST(NOW(6) AS CHAR) AS now');
...
app.use('/api/client-transactions', clientSalesRouter);
app.use('/api/client-transactions', clientSalesImportProfileRouter);
...
app.use('/api/client-transactions', clientStockOpnameRouter);
app.use('/api/client-transactions', clientBomProductionRouter);
...
app.use((err: any, _req: express.Request, res: express.Response, _next: express.NextFunction) => {
  ...
  res.status(status).json({
    error: duplicate ? 'DUPLICATE_ENTRY' : fk ? 'FOREIGN_KEY_VIOLATION' : 'INTERNAL_ERROR',
