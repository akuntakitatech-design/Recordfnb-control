...
clientTransactionRouter.post('/:transactionId/attachments', async (req, res) => {
  ...
  // Key objek storage (R2/local): <workspace>/<transaction>/<uuid>-<nama-aman>
  const storedName = `${crypto.randomUUID()}-${fileName.replace(/[^a-zA-Z0-9._-]+/g, '_')}`;
  const storageKey = `${tx.workspace_id}/${transactionId}/${storedName}`;
  try {
    await storage.put(storageKey, buffer, mimeType);
  } catch (err) {
    console.error('[attachments] upload gagal:', err);
    return res.status(502).json({ error: 'ATTACHMENT_STORAGE_UNAVAILABLE' });
  }
  ...
  try {
    result = await query(
      ...
      [tx.workspace_id, transactionId, fileName, storageKey, mimeType, buffer.length, req.sessionUser!.id],
    );
  } catch (err) {
    // Rollback objek yang sudah terunggah agar tidak jadi sampah di bucket.
...
clientTransactionRouter.delete('/:transactionId/attachments/:attachmentId', async (req, res) => {
  const transactionId = text(req.params.transactionId);
  const attachmentId = text(req.params.attachmentId);
  const tx = await requireTransactionAccess(req.sessionUser!.id, transactionId);
  if (!tx) return res.status(404).json({ error: 'TRANSACTION_NOT_FOUND' });
  if (!(await canCreateTransaction(req.sessionUser!.id, tx.company_id))) return res.status(403).json({ error: 'FORBIDDEN' });
  const attachment = await query<{ storage_path: string }>(
    `SELECT storage_path FROM attachments WHERE id=$1 AND entity_type='TRANSACTION' AND entity_id=$2`,
  ...
  await storage.remove(attachment.rows[0].storage_path).catch(err => console.error('[attachments] hapus objek gagal:', err));
  res.status(204).end();
});
...
clientTransactionRouter.get('/:transactionId/attachments/:attachmentId/file', async (req, res) => {
  ...
  let object;
  try {
    object = await storage.get(row.storage_path);
  } catch (err: any) {
    const code = err?.name || err?.Code || err?.code;
    if (code === 'NoSuchKey' || code === 'NotFound' || code === 'ENOENT' || code === 'INVALID_ATTACHMENT_PATH') {
      return res.status(404).json({ error: 'ATTACHMENT_FILE_NOT_FOUND' });
    }
    console.error('[attachments] download gagal:', err);
    return res.status(502).json({ error: 'ATTACHMENT_STORAGE_UNAVAILABLE' });
  }
  res.type(row.mime_type || object.contentType || 'application/octet-stream');
